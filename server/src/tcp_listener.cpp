...
void TCP_Listener::listenForTCPConnections()
{
    ...
    if (socketFileDescriptor < 0){
		Log::LogEvent(Log::ERROR,"ERROR opening socket");
        exit(1);
    }
    ...
    {
		Log::LogEvent(Log::ERROR,"ERROR on binding");
        exit(1);
    }
    ...
    while(System_Signal::exitActivated() == false){//continue until the system is ready to shut down
		Log::LogEvent(Log::DEBUG,"Listening for for new client connections...");
        ...
        {
			Log::LogEvent(Log::ERROR,"ERROR on new connection accept");
            exit(1);
